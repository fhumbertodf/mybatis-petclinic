...
class ItemMapperTest {
	...
	public ItemMapperTest() {
		...
		mapper = container.select(ItemMapper.class).get();
	...
	void updateInventoryQuantity() {
		...
		// then
		Integer quantity = sqlSession.selectOne("org.mybatis.jpetstore.mapper.ItemMapper.dynamicQuery",
				String.format("SELECT QTY FROM inventory WHERE itemid = '%s'", itemId));
		assertThat(quantity).isEqualTo(9990);
